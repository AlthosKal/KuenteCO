...
import '../../dto/account/profile_detail_dto.dart';
...
class _AccountSelectionScreenState extends State<AccountSelectionScreen> {
  ...
  void _handleSuccessfulResponse(dynamic responseData) {
    if (responseData is List) {
      setState(() {
        accounts = responseData.map((json) => AccountDetailDTO.fromJson(json)).toList();
        isLoading = false;
      });
    } else {
      setState(() {
  ...
  Future<void> _handleLogout() async {
    try {
      ...
      if (mounted) {
        widget.onLogout(); // Usa el callback proporcionado
