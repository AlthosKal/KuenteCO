...
public interface TransactionMapper {
    ...
    default List<TransactionSummaryDTO> convertToTransactionSummaryList(List<?> list) {
        ...
        return list.stream()
                ...
                .filter(userProfile -> userProfile.getProfiles() != null && !userProfile.getProfiles().isEmpty())
                .flatMap(userProfile -> userProfile.getProfiles().stream()
                        .filter(profile -> profile.getTransactions() != null && !profile.getTransactions().isEmpty())
                        .flatMap(profile -> profile.getTransactions().stream()
                                .map(transaction -> mapTransactionToSummary(userProfile, profile, transaction))))
                .collect(Collectors.groupingBy(
                        TransactionSummaryDTO::getCategoryName,
                        Collectors.toList()))
                .entrySet()
                .stream()
    ...
    private TransactionSummaryDTO mapTransactionToSummary(
        ...
        String categoryName = transaction.getDescription() != null &&
        ...
        return TransactionSummaryDTO.builder()
                ...
                .totalIncome(transaction.getType() == TransactionType.INCOME ? transaction.getAmount() : BigDecimal.ZERO)
                .totalExpenses(transaction.getType() == TransactionType.EXPENSE ? transaction.getAmount() : BigDecimal.ZERO)
                .netAmount(transaction.getType() == TransactionType.INCOME ?
    ...
    private TransactionSummaryDTO aggregateTransactionsByCategory(String categoryName, List<TransactionSummaryDTO> transactions) {
        ...
        long totalIncomeCount = transactions.stream()
                .mapToLong(TransactionSummaryDTO::getIncomeCount)
                .sum();

        long totalExpenseCount = transactions.stream()
                .mapToLong(TransactionSummaryDTO::getExpenseCount)
                .sum();
        ...
        return TransactionSummaryDTO.builder()
                ...
                .firstTransactionDate(transactions.stream()
                        .map(TransactionSummaryDTO::getFirstTransactionDate)
                        .min(Comparator.naturalOrder())
                        .orElse(null))
                .lastTransactionDate(transactions.stream()
