...
public class ResponseTypeDetectorServiceImpl implements ResponseTypeDetectorService {
    ...
    public ResponseTypeDetectorServiceImpl(DynamicResponseFactory responseFactory, TransactionMapper transactionMapper) {
    ...
    @Override
    public BaseDynamicResponseDTO detectAndCreateResponse(String prompt, String functionName, Object data) {
        return responseHandlers.getOrDefault(
                functionName,
                (p, d) -> responseFactory.createSimpleTextResponse(p, "Respuesta generada por IA para: " + p)
        ).apply(prompt, data);
    }
    ...
    private Map<String, BiFunction<String, Object, BaseDynamicResponseDTO>> buildHandlers() {
        ...
        map.put("BalanceOverTime", listHandler(TransactionSummaryDTO.class, responseFactory::createBalanceOverTimeResponse));
        map.put("IncomesAndExpensesByPeriod", listHandler(TransactionSummaryDTO.class, responseFactory::createBalanceOverTimeResponse));
        ...
        map.put("projectFinancialBalance", listHandler(TransactionSummaryDTO.class, responseFactory::createFinancialProjectionResponse));

        map.put("suggestExpenseReductions", listHandler(TransactionSummaryDTO.class, responseFactory::createExpenseReductionResponse));

        map.put("compareFinancialPeriods", listHandler(BudgetVsActualDTO.class, responseFactory::createBudgetComparisonResponse));

        map.put("financialStatement", listHandler(BudgetSummaryDTO.class, responseFactory::createBudgetSummaryResponse));
    ...
    private <T> BiFunction<String, Object, BaseDynamicResponseDTO> listHandler(
            Class<T> clazz,
            BiFunction<String, List<T>, BaseDynamicResponseDTO> handler
    ) {
        return (prompt, data) -> {
            ...
            return responseFactory.createSimpleTextResponse(prompt, "No se encontraron datos para el análisis.");
        };
    }
