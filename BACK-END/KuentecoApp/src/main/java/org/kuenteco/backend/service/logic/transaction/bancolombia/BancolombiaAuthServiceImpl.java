...
public class BancolombiaAuthServiceImpl implements BancolombiaAuthService {
    ...
    public String requestNewToken() {
        ...
        log.debug("  - URL: {}{}", authWebClient.toString(), props.getSandbox().getAuth().getTokenUrlBasePath());
        ...
        try {
            tr = authWebClient
                    ...
                    .retryWhen(
                            Retry.backoff(props.getSandbox().getApi().getMaxRetries(), Duration.ofSeconds(1))
                    ...
                    .onErrorMap(TimeoutException.class, ex -> {
                        log.error("⏱️ Timeout solicitando token después de {}s",
                                props.getSandbox().getApi().getTimeoutSeconds());
                        return new BancolombiaTimeoutException("Timeout solicitando token", ex);
                    })
                    .onErrorMap(WebClientResponseException.Unauthorized.class, ex -> {
                        log.error("🔐 Error 401: Credenciales inválidas - Response: {}",
                                ex.getResponseBodyAsString());
                        return new BancolombiaAuthenticationException("Credenciales inválidas", ex);
                    })
                    .onErrorMap(WebClientResponseException.Forbidden.class, ex -> {
                        log.error("🚫 Error 403: Acceso denegado - Response: {}",
                                ex.getResponseBodyAsString());
                        return new BancolombiaAuthorizationException("Acceso denegado", ex);
                    })
                    .onErrorMap(WebClientResponseException.TooManyRequests.class, ex -> {
                        log.error("🚦 Error 429: Rate limit excedido - Response: {}",
                                ex.getResponseBodyAsString());
                        return new BancolombiaRateLimitException("Rate limit excedido", ex);
                    })
                    .onErrorMap(WebClientResponseException.class, ex -> {
                        log.error("❌ Error HTTP {}: {} - Response: {}",
        ...
        log.debug("✅ Token obtenido - Tipo: {}, Expires in: {}s, Scope: {}",
                tr.getTokenType(), tr.getExpiresIn(), tr.getScope());
        log.debug("✅ Token (primeros 20 chars): {}",
    ...
    public void cleanupExpiredTokens() {
        ...
        log.info("🧹 Limpieza completada - Tokens expirados desactivados: {}, eliminados: {}",
