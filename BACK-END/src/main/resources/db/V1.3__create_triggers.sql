...
IF v_total_spent > v_assigned_amount THEN
        ...
        VALUES (
            ...
            NOW(),
            jsonb_build_object(
                'message', 'Has excedido el presupuesto asignado para la categoría ' || (SELECT name FROM Category WHERE id = NEW.id_category),
...
BEGIN
    ...
    IF NEW.expiration_date <= NOW() + INTERVAL '3 days' THEN
        -- Insertar una notificación
        INSERT INTO Notification (id_account, date_send, content)
        VALUES (
            NEW.id_account,
            NOW(),
            jsonb_build_object(
                'message', 'Tu deuda "' || NEW.name || '" vence el ' || NEW.expiration_date || '. Por favor, realiza el pago.',
                'type', 'debt_reminder'
            )
        );
END IF;
