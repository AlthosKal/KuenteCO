...
public class CategoryServiceImpl implements CategoryService {
    ...
    @Override
    @Transactional
    public Category createCategory(Integer idAccount, Integer idAsset, String name, String description,
    ...
    public Category updateCategory(Integer idCategory, String name, String description, BigDecimal assignedBudget,
        ...
        if (assignedBudget != null && !assignedBudget.equals(category.getAssignedBudget())) {
            ...
            if (difference.compareTo(BigDecimal.ZERO) > 0 && budget.getRemainingBudget().compareTo(difference) < 0) {
        ...
        if (assignedBudget != null)
            category.setAssignedBudget(assignedBudget);
        if (startDate != null)
            category.setStartDate(startDate);
        if (finishDate != null)
            category.setFinishDate(finishDate);
    ...
    public Category changeState(Integer idCategory, State state) {
        ...
        if (state == State.INACTIVE) {
            ...
            BigDecimal usedBudget = slaveTransactionRepository.findByCategoryId(idCategory).stream()
    ...
    public CategoryDTO getCategoryStatistics(Integer idCategory) {
        ...
        BigDecimal totalIncome = slaveTransactionRepository.findByCategoryId(idCategory).stream()
                .filter(t -> t.getType() == TransactionType.INCOME).map(Transaction::getAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        // Calcular gastos de la categoría
        BigDecimal totalExpenses = slaveTransactionRepository.findByCategoryId(idCategory).stream()
                .filter(t -> t.getType() == TransactionType.EXPENSE).map(Transaction::getAmount)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        return new CategoryDTO(totalIncome, totalExpenses, balance, category.getAssignedBudget(),
